---
title: "Add a command"
description: "Model a real openparliament record and expose it as a command, a route, and a tool at once."
weight: 10
---

The scaffold ships one example type, `page`. Real work means modelling the
records openparliament actually serves. You do that in two files, and every surface
updates itself.

## 1. Model the record

In `openparliament/openparliament.go`, add a struct for the thing you are fetching
...
type Item struct {
    ...
    Owner string `json:"owner" kit:"link,kind=openparliament/user"` // an edge to another record
}
...
- `kit:"link,kind=<scheme>/<type>"` marks an outbound edge. It can point at
  another openparliament type or at another site entirely, which is what lets a
  host walk the graph across tools.

## 2. Declare the operation

In `openparliament/domain.go`, add an input struct and a handler, then register
it in `Register`:
...
```bash
openparliament item <id>                 # the command
curl 'localhost:7777/v1/item/<id>'      # the route, under serve
ant get openparliament://item/<id>       # the URI dereference, via a host
```
